...
$yacht = DB::table('yachts')->join('Booking', 'Yachts.Yachts_id', 'Booking.Yachts_Yachts_id')->join('users','users.id','=','Booking.Users_id')->join('Marinas','Marinas.Marinas_id','=','Yachts.Yacht_marina')->join('Countries','Countries.Countries_id','=','Marinas.Countries_Countries_id')->where('Countries_id','=',$Countries_id)->where('Yachts.Yachts_id','=',$Yachts_id)->orderBy('Yachts.Yacht_name')->orderBy('Booking.Booking_date_otpr')->get();
$yachtNames = DB::table('yachts')->where('Yachts_id','=',$Yachts_id)->get();
?>
...
<div class="container">
    ...
    @slot('active'){{$oneall->Yacht_name}}@endslot
    @endcomponent
    @break
    @endforeach
</div>
