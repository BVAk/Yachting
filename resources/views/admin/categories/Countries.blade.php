...
$yachtName = DB::table('yachts')->join('Booking', 'Yachts.Yachts_id', 'Booking.Yachts_Yachts_id')->join('users','users.id','=','Booking.Users_id')->join('Marinas','Marinas.Marinas_id','=','Yachts.Yacht_marina')->join('Countries','Countries.Countries_id','=','Marinas.Countries_Countries_id')->where('Countries_id','=',$Countries_id)->orderBy('Yachts.Yacht_name')->orderBy('Booking.Booking_date_otpr')->get();
...
<div class="container">
    @foreach ($marinas as $oneall)

    @component('admin.components.breadcrumb')
    @slot('title') Просмотр заказов @endslot
    @slot('parent') Главная @endslot
    @slot('active'){{$oneall->Countries_name}} @endslot
    @endcomponent
    @break
    @endforeach
</div>
        ...
        <div class="col-md-8 col-md-offset-2">
            <div class="panel panel-default">
                <div class="links" align="center">
                    @foreach ($marinas as $oneall)
                    <a href="{{$oneall->Countries_id}}/{{$oneall->Marinas_id}}"
                     style="padding:20px">{{$oneall->Marinas_name}} </a>
                     @endforeach
                 </div>
